...
<div class="Create">
    <form action="{{ route('event_announcements.store') }}" method="POST">
        ...
        <div>
            ...
            <input type="date" name="event_date" id="event_date" min="1900-01-01" required>
        </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        // Set min date to today for scheduling (events can be future dates)
        const today = new Date().toISOString().split('T')[0];
        const eventDateInput = document.getElementById('event_date');
        
        if (eventDateInput) {
            eventDateInput.min = today; // Events can be scheduled for future
        }
        ...
        if (eventDateInput) limitYearInput(eventDateInput);
        ...
        if (form && submitButton) {
            form.addEventListener('submit', function(e) {
                ...
                if (confirm('Save this announcement?')) {
